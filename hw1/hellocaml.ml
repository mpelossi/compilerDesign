...
(*
  ...
  homepage -- there are links to the standard libraries, and online tutorials.
  ...
  Also, please feel free to ask TAs or on MoodleOverlow for help if you get stuck.
  Learning OCaml is essential for your successful completion of later projects, but 
  it is not supposed to be too onerous in itself!
*)
...
(*
  OCaml is strongly typed and provides a standard suite of base types:
    int  -  31 or 63 bit integers 0, 1, (-1), etc., (depending on your system)
...
(*
  ...
  and choose the setup according to how you would like to interact with OCaml.
...
let rec optimize (e:exp) : exp =
  begin match e with
    ...
    | Const x -> Const x
    | Add(x,y) ->
      let x_opt = optimize x in
      let y_opt = optimize y in
      begin match (x_opt, y_opt) with
        | (_, Const 0L) -> x_opt
        | (Const 0L, _) -> y_opt
        | (Const a, Const b) -> Const (Int64.add a b)
        | (Neg c, Neg d) -> Add(c, d)
        | (c, Neg d) ->
          if c = d then
            Const 0L
    ...
    | Mult(x,y) ->
      ...
      begin match (x_opt, y_opt) with
        ...
        | (_, Const 1L) -> x_opt
        | (Const 1L, _) -> y_opt
        | (Const a, Const b) -> Const (Int64.mul a b)
        | (Neg c, Neg d) -> Mult(c, d)
        | _ -> Mult(x_opt, y_opt) (* 'else' *)
      end  
    | Neg x ->
      let x_opt = optimize x in
      begin match x_opt with
        | Const a -> Const (Int64.neg a) 
        | Neg a -> optimize a (* double negation *)
        | _ -> Neg x_opt (* 'else' *)
      end
  end
