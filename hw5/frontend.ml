...
let oat_alloc_struct ct (id: Ast.id) : Ll.ty * operand * stream =
  let struct_var, raw_ptr = gensym "struct_var", gensym "raw_ptr" in
  let struct_type = cmp_ty ct (TRef (RStruct id)) in
  let ptr_type = Ptr I64 in
  let fields = TypeCtxt.lookup id ct in
  let total_size = Const (Int64.mul (Int64.of_int (List.length fields)) 8L) in
  struct_type, Id struct_var, lift
    [ raw_ptr, Call(ptr_type, Gid "oat_malloc", [I64, total_size])
    ; struct_var, Bitcast(ptr_type, Id raw_ptr, struct_type) ]
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length expr ->
    let array_type, array_operand, array_code = cmp_exp tc c expr in
    let elem_type = 
      match array_type with
      | Ptr (Struct [I64; Array (_, et)]) -> et
      | _ -> failwith "Length can only accept Arrays"
    in
    let gep_pointer = gensym "gep_pointer" in
    let length_var = gensym "length_var" in
    I64, Id length_var, array_code >@ lift [ 
      gep_pointer, Gep(array_type, array_operand, [Const 0L; Const 0L]);
      length_var, Load(Ptr elem_type, Id gep_pointer)
    ]
  ...
  *)
  | Ast.NewArr (elt_ty, e1, id, e2) ->    
    ...
    let arr_ty, arr_op, alloc_code = oat_alloc_array tc elt_ty size_op in
    let array_ptr = gensym "array_ptr" in
    let length_ptr = gensym "length_ptr" in 
    let result_ptr = gensym "result_ptr" in
    let updated_ctx = 
      Ctxt.add (Ctxt.add c length_ptr (Ptr I64, Id length_ptr)) array_ptr (Ptr arr_ty, Id array_ptr) 
    in
    let allocate_array = lift [ 
      array_ptr, Alloca arr_ty; 
      "", Store (arr_ty, arr_op, Id array_ptr)
    ] in 
    let store_length = lift [ 
      length_ptr, Alloca I64; 
      "", Store (I64, size_op, Id length_ptr)
    ] in
    let loop_decl = (id, no_loc @@ CInt 0L) in
    let loop_cond = no_loc @@ Bop(Lt, no_loc @@ Id id, no_loc @@ Id length_ptr) in
    let loop_increment = no_loc @@ Assn(no_loc @@ Id id, no_loc @@ Bop(Add, no_loc @@ Id id, no_loc @@ CInt 1L)) in
    let loop_body_assignment = no_loc @@ Assn(no_loc @@ Index(no_loc @@ Id array_ptr, no_loc @@ Id id), e2) in
    let loop = no_loc @@ For([loop_decl], Some loop_cond, Some loop_increment, [loop_body_assignment]) in 
    let _, loop_code = cmp_stmt tc updated_ctx Void loop in 
    arr_ty, Id result_ptr,
    size_code >@ alloc_code >@ allocate_array >@ store_length  >@ loop_code>@ lift [result_ptr, Load(Ptr arr_ty, Id array_ptr)] 
   ...
   *)
   | Ast.CStruct (struct_id, field_list) ->
    let struct_type, struct_op, struct_stream = oat_alloc_struct tc struct_id in
    let handle_field = 
      (fun (stores, cmps) (field_name, field_expr) ->
        let field_type, field_idx = TypeCtxt.lookup_field_name struct_id field_name tc in
        let expr_type, expr_op, expr_stream = cmp_exp tc c field_expr in
        let llvm_field_type = cmp_ty tc field_type in
        let field_ptr = gensym (field_name ^ "_ptr") in
        let field_val = gensym (field_name ^ "_val") in
        let store_instr = gensym ("store_" ^ field_name ^ "_val") in
        let gep_instr = [I (field_ptr, Gep (struct_type, struct_op, [Const 0L; Const field_idx]))] in
        let cast_instr = [I (field_val, Bitcast (expr_type, expr_op, llvm_field_type))] in 
        let store_instr_list = [I (store_instr, Store (llvm_field_type, Id field_val, Id field_ptr))] in
        (stores >@ gep_instr >@ cast_instr >@ store_instr_list, cmps >@ expr_stream)
      )
    in
    let store_stream, cmp_stream = List.fold_left handle_field ([], []) field_list in
    struct_type, struct_op, struct_stream >@ cmp_stream >@ store_stream
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (expr, field) ->
    let struct_ty, struct_op, struct_stream = cmp_exp tc c expr in
    let struct_id =
      match struct_ty with
      | Ptr (Namedt id) -> id
      | _ -> failwith "Invalid type for struct projection"
    in
    let field_type, field_idx = TypeCtxt.lookup_field_name struct_id field tc in
    let llvm_field_ty = cmp_ty tc field_type in
    let gep_var = gensym (field ^ "_gep") in
    let gep_stream = [I (gep_var, Gep (struct_ty, struct_op, [Const 0L; Const field_idx]))] in
    (llvm_field_ty, Id gep_var, struct_stream @ gep_stream)
  ...
  *)
  | Ast.Index (e, i) ->
    ...
    let ptr_id, len_ptr_id = gensym "index_ptr", gensym "len_ptr" in
    (* Compute the pointer to the length field *)
    let len_ptr_code = [I (len_ptr_id, Gep (arr_ty, arr_op, [i64_op_of_int 0; i64_op_of_int 0]))] in
    (* Call oat_assert_array_length with the correct length pointer *)
    let assert_len_stream = [I ("", Call (Void, Gid "oat_assert_array_length", [Ptr I64, Id len_ptr_id; I64, ind_op]))] in
    ans_ty, (Id ptr_id),
    arr_code >@ ind_code >@ len_ptr_code >@ assert_len_stream >@ lift
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (cast_type, var_id, expression, then_block, else_block) ->
    let expr_ty, expr_op, expr_code = cmp_exp tc c expression in
    let null_lbl = gensym "null_lbl" in
    let notnull_lbl = gensym "notnull_lbl" in
    let merge_lbl = gensym "merge_lbl" in
    let null_branch = cmp_block tc c rt else_block in
    let notnull_branch = cmp_block tc (Ctxt.add c var_id (Ptr expr_ty, Id var_id)) rt then_block in
    let condition_id = gensym "cond_id" in
    c, expr_code >:: I (condition_id, Icmp (Ne, expr_ty, expr_op, Null)) >:: T (Cbr (Id condition_id, notnull_lbl, null_lbl))
    >:: L notnull_lbl >:: E(var_id, Alloca expr_ty) >:: I("", Store (expr_ty, expr_op, Id var_id)) >@ notnull_branch >:: 
    T(Br merge_lbl) >:: L null_lbl >@ null_branch >:: T(Br merge_lbl) >:: L merge_lbl
...
let sort_fields (tc: TypeCtxt.t) (struct_id: id) (fields: (id * exp node) list) : (id * exp node) list =
  List.sort (fun (fname1, _) (fname2, _) ->
    TypeCtxt.index_of_field struct_id fname1 tc - TypeCtxt.index_of_field struct_id fname2 tc
  ) fields
...
| CStruct (struct_id, components) ->
    let initializer_symbol = gensym (struct_id ^ "_init") in
    let llvm_struct_type = Namedt struct_id in
    let ordered_fields = sort_fields tc struct_id components in
    let declarations, additional_globals = List.fold_right (
      fun (_, expr) (decls_acc, globals_acc) ->
        let decl, new_globals = cmp_gexp c tc expr in
        decl :: decls_acc, globals_acc @ new_globals
    ) ordered_fields ([], [])
    in
    let struct_init = GStruct declarations in
    (
      (Ptr llvm_struct_type, GGid initializer_symbol), 
      (initializer_symbol, (llvm_struct_type, struct_init)) :: additional_globals
    )
