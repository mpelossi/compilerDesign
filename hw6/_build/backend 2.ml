...
(* A simple standard coloring-based register allocator for better_layout *)

module UidS = Datastructures.UidS
module UidM = Datastructures.UidM

(* The set of available registers for allocation (excluding Rax and Rcx) *)
let available_registers : Alloc.loc list =
  [Alloc.LReg Rdi; Alloc.LReg Rsi; Alloc.LReg Rdx; Alloc.LReg R08; 
   Alloc.LReg R09; Alloc.LReg R10; Alloc.LReg R11]

(* Build an interference graph:
   For each uid in the function, all the uids live at that point interfere 
   pairwise. We'll store the interference graph as a map from uid to a set of 
   all other uids it interferes with. *)
let build_interference_graph (f:Ll.fdecl) (live: Liveness.liveness) : UidS.t UidM.t =
  let add_interference s graph =
    (* Add edges for all pairs in s *)
    UidS.fold (fun u acc ->
      let neighbors = UidS.remove u s in
      UidM.add u (match UidM.find_opt u acc with
                  | None -> neighbors
                  | Some old -> UidS.union old neighbors)
        acc
    ) s graph
  in
  (* Fold over all instructions and the terminator to add interference edges *)
  let graph = 
    Llutil.fold_fdecl
      (fun g (x,_) -> if x <> "" then UidM.add x UidS.empty g else g)
      (fun g lbl -> UidM.add lbl UidS.empty g)
      (fun g (x,_) -> UidM.add x UidS.empty g)
      (fun g _ -> g)
      UidM.empty f
  in
  ...
  let graph =
    Llutil.fold_fdecl
      (fun g (x,_) -> g)
      (fun g _ -> g)
      (fun g (x,_) -> add_interference (live.live_in x) g)
      (fun g (x,_) -> add_interference (live.live_in x) g)
      graph f
  in
  graph

(* Simplify/Spill phase of graph coloring:
   We repeatedly remove nodes with degree < k (k = number of available registers)
   If no such node is found, we pick a node to spill.
   We return a stack of nodes in the order they were removed (for coloring later).
*)
let k = List.length available_registers

let rec simplify_spill graph stack =
  if UidM.is_empty graph then stack
  else
    let low_degree_nodes = UidM.filter (fun _ neigh -> UidS.cardinal neigh < k) graph in
    if not (UidM.is_empty low_degree_nodes) then
      (* Pick an arbitrary low-degree node *)
      let (node, neighbors) = UidM.choose low_degree_nodes in
      let graph' =
        UidM.remove node (UidM.map (fun s -> UidS.remove node s) graph)
      in
      simplify_spill graph' (node::stack)
    else
      (* No node with degree < k; must spill *)
      let (node, neighbors) = UidM.choose graph in
      let graph' =
        UidM.remove node (UidM.map (fun s -> UidS.remove node s) graph)
      in
      simplify_spill graph' (node::stack)
...
(* Select colors for nodes in reverse order they were removed. 
   We pick the first available register not used by its neighbors.
   If no register is available, we spill it.
 *)
let assign_colors stack original_graph =
  let rec aux stack color_map =
    match stack with
    | [] -> color_map
    | u::rest ->
      let neighbors = UidM.find_opt u original_graph |> Option.value ~default:UidS.empty in
      let used =
        UidS.fold (fun n s ->
          match UidM.find_opt n color_map with
          | Some loc -> LocSet.add loc s
          | None -> s
        ) neighbors LocSet.empty
      in
      let rec find_reg = function
        | [] -> None
        | r::rs -> if LocSet.mem r used then find_reg rs else Some r
      in
      let chosen = find_reg available_registers in
      let chosen_loc = match chosen with Some c -> c | None -> (* Spill *) Alloc.LStk (-1) in
      aux rest (UidM.add u chosen_loc color_map)
  in
  aux stack UidM.empty

let better_layout (f:Ll.fdecl) (live:Liveness.liveness) : layout =
  let graph = build_interference_graph f live in

  (* We must include all function parameters, labels, and assigned uids in the map *)
  let all_uids = 
    Llutil.fold_fdecl
      (fun s (x,_) -> UidS.add x s)
      (fun s l -> UidS.add l s)
      (fun s (x,i) -> 
         if Llutil.insn_assigns i then UidS.add x s else s)
      (fun s (x,_) -> UidS.add x s)
      UidS.empty f
  in
  let graph =
    UidS.fold (fun u g -> if UidM.mem u g then g else UidM.add u UidS.empty g) all_uids graph
  in

  let stack = simplify_spill graph [] in
  (* stack is in reverse order of removal, which is what we want *)

  let color_map = assign_colors stack graph in

  (* We must also allocate locations for parameters and labels not assigned by instructions *)
  let n_spill = ref 0 in
  let arg_count = ref 0 in

  let arg_loc (i:int) : Alloc.loc =
    match Backend.arg_reg i with
    | Some r -> Alloc.LReg r
    | None -> incr n_spill; Alloc.LStk (- !n_spill)
  in

  (* Assign locations to parameters if not already in color_map *)
  let param_locs = List.map (fun (p,_) ->
    let loc = match UidM.find_opt p color_map with
      | Some l -> l
      | None ->
        let l = arg_loc !arg_count in
        incr arg_count;
        l
    in
    (p, loc)
  ...
  (* Assign locations to labels *)
  let entry, bs = f.f_cfg in
  let all_labels = List.map fst bs @ [Llutil.label_of_block entry] in
  let label_locs = List.map (fun l ->
    match UidM.find_opt l color_map with
    | Some loc -> (l, loc)
    | None -> (l, Alloc.LLbl (Platform.mangle l))
  ) all_labels in

  (* For uids assigned by instructions:
     If they don't have a color, spill them.
   *)
  let insn_uids =
    Llutil.fold_fdecl
      (fun s _ -> s) (fun s _ -> s)
      (fun s (x,i) -> if Llutil.insn_assigns i then UidS.add x s else s)
      (fun s _ -> s)
      UidS.empty f
  in

  let insn_locs = UidS.fold (fun u acc ->
    let loc = match UidM.find_opt u color_map with
      | Some (Alloc.LStk(-1)) ->
  ...
  let all_locs = param_locs @ label_locs @ insn_locs in
  let uid_loc x =
    match List.assoc_opt x all_locs with
    | Some l -> l
    | None ->
      (* if not found, maybe it's not a uid that needs a location *)
      Alloc.LVoid
  in

  { uid_loc; spill_bytes = 8 * !n_spill }
