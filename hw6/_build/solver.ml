...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) =
  struct
    (* We use a helper function to iteratively process the worklist *)
    let rec iterate (worklist : Graph.NodeS.t) (g : Graph.t) : Graph.t =
      if Graph.NodeS.is_empty worklist then g
      else begin
        let n = Graph.NodeS.choose worklist in
        let w_rest = Graph.NodeS.remove n worklist in

        (* Combine facts of all predecessors *)
        let preds_of_n = Graph.preds g n in
        let incoming_facts =
          Graph.NodeS.fold (fun p acc -> (Graph.out g p)::acc) preds_of_n [] 
        in
        let in_fact = Fact.combine incoming_facts in

        (* Apply the flow function *)
        let new_out = Graph.flow g n in_fact in
        let old_out = Graph.out g n in

        (* If changed, update graph and add successors *)
        if Fact.compare new_out old_out <> 0 then
          let g' = Graph.add_fact n new_out g in
          let succs_of_n = Graph.succs g n in
          let new_w = Graph.NodeS.union w_rest succs_of_n in
          iterate new_w g'
        else
          iterate w_rest g
      end
    ...
    let solve (g:Graph.t) : Graph.t =
      let initial_w = Graph.nodes g in
      iterate initial_w g
  end
