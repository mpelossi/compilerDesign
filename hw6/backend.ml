...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  let n_arg = ref 0 in
  let act_arg = ref 0 in
  let n_spill = ref 0 in
  let spill () = (incr n_spill; Alloc.LStk (- !n_spill)) in
  let regs_arg () =
    let res =
      match arg_loc !act_arg with
        | Alloc.LReg Rcx -> -2
        | Alloc.LReg Rdi -> 0
        | Alloc.LReg Rsi -> 1
        | Alloc.LReg Rdx -> 2
        ...
        | Alloc.LReg R09 -> 4
        | Alloc.LReg R10 -> 5
        | Alloc.LReg Rax | Alloc.LReg R11 -> 6
      | x -> -2
    in
    ignore (incr act_arg); res
  in
  let regs_av = [Alloc.LReg Rdi;Alloc.LReg Rsi;Alloc.LReg Rdx;Alloc.LReg R08;Alloc.LReg R09;Alloc.LReg R10;Alloc.LReg R11]in
  let (_,regs_map) = List.fold_left (fun (act, map) reg -> (act + 1, Datastructures.UidM.add (string_of_int act) reg map)) (0, Datastructures.UidM.empty) regs_av
  in
  let regs_map = Datastructures.UidM.add (string_of_int (-5)) Alloc.LVoid regs_map
  in
  let uids = 
    fold_fdecl
      (fun lo (x, _) -> Datastructures.UidM.add x (regs_arg ()) lo)
      (fun lo l -> lo)
      (fun lo (x, i) ->
        let lo_upd2 = 
        begin match i with
          | Call (ty, op, args) ->
              let (_ ,lo_upd1) = List.fold_left (fun (act, map) (_ , op) -> 
                begin match op with
                  | Id id ->
                      if ((Datastructures.UidM.find id map) = -1 && (act = 0 || act = 1 || act = 2))
                        then act + 1, Datastructures.UidM.add id (act - 10) map
                      else (if ((Datastructures.UidM.find id map) = -1 && (act = 4 || act = 5))
                              then (act + 1, Datastructures.UidM.add id (act - 11) map)
                            else
                              (act + 1, map))
                  | _ -> (act + 1, map)
                end
        ...
        if insn_assigns i 
        then Datastructures.UidM.add x (-1) lo_upd2
        else lo_upd2)
      (fun lo _ -> lo)
      Datastructures.UidM.empty f in
  let graph =
    let empty_graph =
      Datastructures.UidM.fold (fun id col map ->
        Datastructures.UidM.add id UidSet.empty map) uids Datastructures.UidM.empty in
    Datastructures.UidM.fold (fun id color map ->
    if(color = -1 || color = -10 || color = -9 || color = -8 || color = -7 || color = -6 ) 
      then(
        let live_inS = live.live_in id in
        let live_outS = live.live_out id in
        let aux_in id map = 
          begin match Datastructures.UidM.find_opt id map with
            | Some s ->
                Datastructures.UidM.add id (UidSet.union s live_inS) map
            | None -> Datastructures.UidM.add id live_inS map
          end in   
        let aux_out id map = 
          begin match Datastructures.UidM.find_opt id map with
            | Some s ->
                Datastructures.UidM.add id (UidSet.union s live_outS) map
            | None -> Datastructures.UidM.add id live_outS map
          end in   
        let map_in = UidSet.fold aux_in live_inS map in
        UidSet.fold aux_out live_outS map_in)
    else
      map
    ) uids empty_graph in
  let graph_new = Datastructures.UidM.mapi (fun node edges -> UidSet.remove node edges) graph in
  let compare_neighbours (id1, _) (id2, _) =
    let neighb1 = Datastructures.UidM.find id1 graph_new in
    let neighb2 = Datastructures.UidM.find id2 graph_new in
    let card1 = UidSet.cardinal neighb1 in
    let card2 = UidSet.cardinal neighb2 in
    if(card1 = card2)
      then 0
    else 
      (if card1 > card2
        then 1
          ...
          0) in
  let rec color_graph graph act nodes list_nodes =
    let list_length = List.length list_nodes in
    if (act = list_length) then nodes
    else
       (let (id, col) = List.nth list_nodes act in 
        let neighb = Datastructures.UidM.find id graph in
        let color_new = 
          if((col = -1 || col = -10 || col = -9 || col = -8 || col = -7 || col = -6) && (not (UidSet.is_empty neighb)))
            then 
              (
              let col_list = (UidSet.fold 
                  (fun n col_list -> 
                    let color_act = Datastructures.UidM.find n nodes in
                    if(color_act = -2 || color_act = -10 ||  color_act = -9 ||  color_act = -8 || 
                        color_act = -7 || color_act = -6)
                      then col_list
                    else
                      col_list@[color_act])
                neighb []) in
              let pref_col_list = (UidSet.fold 
                  (fun n col_list -> 
                    let color_act = Datastructures.UidM.find n nodes in
                    if(color_act = -10 || color_act = -9 || color_act = -8 || 
                        color_act = -7 || color_act = -6)
                      then col_list@[color_act + 10]
                    else
                      col_list)
                neighb []) in
              let sort_col = List.sort compare col_list in
              let sort_pref_col = List.sort compare pref_col_list in
              let rec get_col list = 
                begin match list with
                  | x::y::xs -> if (x + 1 < y) then x + 1 else get_col (y::xs)
                  | [x] -> x + 1
                  | [] -> 0
                end in
              let rec get_col_cons_pref list = 
                begin match list with
                  | x::y::xs ->
                      if (x + 1 < y && List.find_opt (fun z -> z = (x + 1)) pref_col_list = None) then x + 1 else  get_col (y::xs)
                  | [x] -> x + 1
                  | [] -> 7
                end in
              let rec check_pref list = 
                if (col <> -1)
                  then(
                    begin match list with
                      | [] -> col + 10
                      | x::xs -> (if (x = col + 10) then get_col sort_col else check_pref xs)
                    end)
                else
                  let pref_col = get_col_cons_pref sort_pref_col in
                  if (pref_col > 6)
                    then get_col sort_col
                  else 
                      (if (List.find_opt (fun x -> x = pref_col) sort_col = None) then pref_col else get_col sort_col)
                       in
                check_pref sort_col
              )
          else 
            (if ((col = -1) && UidSet.is_empty neighb) then 0
            else 
              (if ((col = -10 || col = -9 || col = -8 || col = -7 || col = -6) && UidSet.is_empty neighb) then col + 10 else col))
          in
        let upd_uids = Datastructures.UidM.mapi (fun n color -> if (n = id) then color_new else color) nodes in
        let sorted_new_uid_list = List.sort compare_neighbours (Datastructures.UidM.bindings upd_uids) in
        color_graph graph (act + 1) upd_uids sorted_new_uid_list           
  ) in
  let sorted_uid_list = List.sort compare_neighbours (Datastructures.UidM.bindings uids) in
  let uids_colors = color_graph graph_new 0 uids sorted_uid_list in
  let allocate lo uid =
    let loc =
      let col = Datastructures.UidM.find uid uids_colors in
      try
        Datastructures.UidM.find (string_of_int col) regs_map
      with
      | Not_found -> spill ()
    in
    Platform.verb @@ Printf.sprintf "allocated: %s <- %s\n" (Alloc.str_loc loc) uid; loc
